...
/**
 * Format last seen status with precise timing
 * Shows "Online" for recent activity, and gradually more general time references
 * as the time since last activity increases
 */
export function formatLastSeen(lastSeen: Date | string | null | undefined): string {
  ...
  if (diffSeconds < 604800) {
    return `Last seen on ${format(lastSeenDate, 'EEEE')} at ${format(lastSeenDate, 'h:mm a')}`;
  }
